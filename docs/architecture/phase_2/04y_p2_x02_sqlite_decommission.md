...
Status: `proposed` (kein Arbeitspaket begonnen)
Erstellt: 2026-08-06 · Entscheidungsgrundlage aktualisiert 2026-08-06
...
| Gehostetes Supabase? | **Erst zum Shipping.** Bis dahin lokaler Stack | `DEC-015`..`DEC-017` bleiben gegated |
...
6. `NEXIMMO_DATA_BACKEND` aus `phase_1/01_environment_contract.md` und
   `app_environment.dart`; `DataBackend` kollabiert
...
| AP | Inhalt | Gate |
|---|---|---|
| `AP-X02-1` | Dokumentenpflege an `DEC-024` angleichen: Charter Z. 21 (Offline-Zusage **und** das dort vorgeschriebene Zwei-Adapter-Muster `legacy_sqlite_*` + `supabase_*`), `CLAUDE.md` (`offline-first`, Adapter-Pattern-Abschnitt) | Charter, `CLAUDE.md` und Register widerspruchsfrei; kommende Wellen bauen nachweislich nur noch einen Adapter je Domäne |
| `AP-X02-2` | **Früher Schnitt (V1).** `DataBackend.sqlite` streichen; Legacy-Adapter, Cutover-Mapper, Cutover-Skripte und `AP4`-Rest entfernen; `P2-X01-AP4` als *nicht anwendbar* schließen | `flutter analyze`/`test` grün; CI-Job `database` unverändert; kein `legacy_*`-Adapter mehr im Baum |
...
| Lokale Auth fällt mit SQLite, bevor die Cloud-Auth alle Rollen abdeckt | `AP-X02-4` (`adminUsers`) liegt vor Strom C Schritt 3; `DEC-016` bleibt offen für die privilegierte Rollenmatrix |
| Gehosteter Betrieb wird vorweggenommen | `DEC-015`..`017` bleiben eigenständig; dieser Plan endet beim lokalen Stack |
...
- `NEXIMMO_DATA_BACKEND` aus Environment-Contract und Code entfernt
...
- Charter und `CLAUDE.md` beschreiben das Produkt nicht mehr als offline-first
- Vor Shipping: `DEC-015`..`DEC-017` entschieden
