...
| DEBT-TOKEN-001 | Two parallel color-token sources — `AppColors` (static, ~61 files) and `_lightTokens`/`_darkTokens` in `lib/ui/theme/app_theme.dart` — must be edited together; several `property_detail` screens also hardcode neutral hex literals instead of reading the theme (`overview_screen.dart`, `property_shell.dart`, `units_screen.dart` and text-only in `budget_vs_actual_screen.dart`, `maintenance_screen.dart`). Collapse to one token source; replace hardcoded hex with theme references. Found during Wave 0 token refinement. | resolve per screen in its wave (1/3/6); collapse token sources in Wave 0 dark-theme follow-up |
