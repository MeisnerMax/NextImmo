...
| P2-X01 | Replace the Supabase-only Reference-Slice host with the canonical authenticated `AppScaffold`; converge additive cloud routes, declare per-platform backend readiness honestly and close data/parity gaps without exposing server secrets | AP0–AP3 before Wave 6; AP4–AP6 before Phase-2 completion |
