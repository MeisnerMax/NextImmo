...
- **Properties (list view)** — first screen through the layout audit. Fixed: KPI tile structure and
  ...
  result count, table density. The final viewport pass replaced the desktop/tablet whole-page
  scroll with a bounded content region (table/cards scroll locally), while phone deliberately keeps
  the stacked whole-page scroll. The filler subtitle was shortened.
...
- **Property overview** (`overview/`) — measured clean on colour (zero hex literals, zero raw
  ...
  `AppChartPalette`.
- **Shell chrome** — `Sidebar` and `TopBar` now use the canonical `NxGlassPanel`; the sidebar keeps
  its deliberately darker navy fill and both surfaces own their single edge contour, without the
  former duplicate `Divider`s in `AppScaffold`.
- **Dark-led startup** — new/incomplete settings records and the app bootstrap default to `dark`.
  `light` and `system` remain explicit choices in Appearance settings.

**Why parts of the property-detail area still look unmigrated**

After the maintenance/leases/scenarios wave, `lib/ui/screens/property_detail/` still contains 34
raw Material `Card(...)` and 8 `fontSize:` matches. One of those type-size matches belongs to the
generated PDF, not the Flutter UI. The largest remaining card clusters are `inputs_screen` and
`operations_overview_screen`; the largest local UI-type clusters are `tenant_detail_screen` and
`rent_roll_screen`.
...
- **Budget vs. Ist** (`budget_vs_actual_screen.dart`, 3684 lines, 7 tabs) — third screen through the
  ...
  The follow-up pass removed the remaining 14 raw Material cards and 15 local Flutter type-size
  overrides. Finance summaries, profit/liquidity KPIs, LTV state and budget panels now use the
  shared card, KPI and status components. Chart labels inherit the theme instead of pinning pixels.

- **Asset-Workbook** (`asset_workbook_screen.dart`) — property, source, cost, settlement and table
  surfaces now use `NxCard`; all in-app KPI forks use `NxKpiTile`/`NxKpiRow`. Error and empty states
  use `NxEmptyState` without raw exception text. Phone/tablet scroll the complete stacked workbook,
  desktop keeps a bounded tab-content region, and dialog widths now use `ResponsiveConstraints`.
  Breakpoint tests cover the Betriebskosten tab at 390×844, 1024×768 and 1440×900. The sole local
  `fontSize` left in the file belongs to the exported PDF note and is intentionally independent of
  the Flutter theme.

- **Instandhaltung** (`maintenance_screen.dart`, 3915 lines) — fourth screen through the audit, and
  ...
  The follow-up pass replaced all 10 remaining Material cards and both local type-size overrides.
  CapEx summaries now use `NxKpiRow`/`NxKpiTile`; renovation, warranty, timeline and component
  surfaces use `NxCard`, and the empty timeline uses `NxEmptyState`.

- **Mietverträge** (`leases_screen.dart`) — the local KPI-card fork and all six local type-size
  overrides were removed in favour of `NxKpiRow`/`NxKpiTile`, `NxStatusBadge` and theme typography.
  The filter dropdown now expands its selected item correctly, and the month picker uses a compact
  calendar affordance so the toolbar remains overflow-free on phone widths.

- **Szenarien & Bewertung** (`scenarios_screen.dart`) — the remaining two Material cards and six
  local type-size overrides now use `NxCard`, `NxKpiTile` and theme typography. Summary figures,
  governance checks, chart-segment labels and legends therefore share the system components and
  text scale.
...
- **Einheiten** (`units_screen.dart`) — fifth screen through the audit. All 10 hex literals gone
  ...
  - The remaining list, detail and pipeline surfaces now use `NxCard`; local type-size overrides
    were removed in favour of the text theme. `unit_detail_screen.dart` now uses `NxKpiTile` and an
    actionable `NxEmptyState` instead of a KPI fork and raw exception text.

- **Handwerker-Stammdaten** (`contractors_screen.dart`) — raw cards and local type-size forks were
  replaced by `NxCard`, `NxKpiRow`/`NxKpiTile`, `NxStatusBadge` and theme typography. The former
  fixed master/detail row is now phone list→detail navigation, a stacked tablet split and a desktop
  two-pane layout. A widget test guards all three viewport classes.
...
- **Final sweep — raw `Card` and `fontSize`** (2026-08-04). `lib/ui` is now at **2 raw `Card(`** (one
  is a comment, one is in the v2 track) and **4 `fontSize:` literals**, all four legitimate:
  `NxKpiTile` defining its own value, two `pw.TextStyle` in PDF export, and the theme's tooltip.
  Down from 54 and 25.

  Two findings worth recording:
  - `lib/ui/shell/v2/sidebar_v2.dart` carried the *previous* sidebar palette as four hardcoded hex
    constants (`#030C28`, `#17417D`, `#EAF2FF`, `#BFD0EA`) — the same class of defect as the screens,
    hiding in the shell. Now on `AppColors.sidebar*`.
  - The whole `v2` track (`shell/v2/`, `screens/v2/`) has **zero external references**. It was left
    in place rather than deleted because `CLAUDE.md` documents it as an in-progress redesign track —
    but it is not wired to anything, and it should either be adopted or retired deliberately.

  **Measurement caveat:** counting raw Material cards with `[^x]Card\(` over-reports badly — it
  matches the tail of `_buildMetricCard(`, `NxFormSectionCard(` and similar. Use a word boundary
  (`(?<![A-Za-z0-9_])Card\(`, .NET/PCRE — ripgrep needs `--pcre2`). An earlier count of "320" was
  this mistake; the real figure was 54.
...
**Open**
- Per-screen structural work beyond the completed Wave-1/Wave-2 surfaces and the implemented
  Wave-5 slices remains governed by `04_screen_redesign_wave_plan.md`; the colour baseline is
  universal, but raw Material `Card` and local `fontSize` overrides still exist in legacy screens.
