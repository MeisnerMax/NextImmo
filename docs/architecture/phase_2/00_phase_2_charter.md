...
- `DEC-005`: SQLite bleibt als Legacy-Quelle und optionaler späterer Client-Cache erhalten — es wird **nicht** hart entfernt, solange keine Migrations-/Paritätsfreigabe vorliegt (`MIG-BND-001`).
- Das bedeutet konkret: Für jede Domäne entsteht **derselbe Adapter-Bruch wie bei Property** — ein `legacy_sqlite_*_adapter` (nur Lesen/Dry-Run-Migration, analog `P1-006`/`P1-012`) und ein `supabase_*_adapter` (produktiver Schreibpfad, analog `P1-004`/`P1-007`). Die App bleibt startfähig ohne Internet für den Übergangszeitraum; produktive Mutation läuft nach Migration einer Domäne ausschließlich über Supabase.
