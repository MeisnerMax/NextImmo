...
| DEC-SEC-002 | **partial** (Baseline accepted 2026-08-08) | Objekt-/Portfolio-Scope-Vererbung bei Mehrfachzuordnung | **Baseline:** `entity_scopes.entity_type` ist eine Allowlist; direkt scopebar sind nur Root-Entitaeten. Keine Scope-Zeilen = workspace-weit; sobald eine existiert, gilt nur noch ein expliziter Treffer; ein Scope ersetzt nie eine fehlende Workspace-Permission. Child-Entitaeten (unit, lease, maintenance_ticket, capex_project, document) bekommen **keine** eigenen Scopes, ihr Zugriff folgt der Parent-Property. **Umgesetzt in PH-01 fuer `property`** (Migration `20260808120000`). `portfolio` ist in der Baseline vorgesehen, aber **noch nicht erlaubt**: es gibt keine `portfolios`-Tabelle und keine Property→Portfolio-Beziehung, ein solcher Scope waere schreibbar und wirkungslos. Er kommt mit `P2-D09` samt Vererbungsregel | Offen bleiben: Mehrfachzuordnung eines Child-Objekts, feinere Sub-Property-Scopes, Entitaeten ohne eindeutige Property-Zuordnung |
