...
- **`REMOTE-SECURITY-GATE-01` ist das naechste erforderliche Paket und blockiert Staging-Auth.** Die 65 `WARN` sind kein Phase-3-Migrationsblocker und kein Schema-Drift, aber vor jeder Freigabe von Staging-Auth bzw. authentifizierten synthetischen Nutzern vollstaendig zu bewerten: Schemaparitaet lokal/remote beweist nur, dass die Migration nichts veraendert hat, nicht dass die Grants sicher sind. Zu pruefen sind mindestens Function Owner, `SECURITY DEFINER`, `search_path`, `EXECUTE`-Grants an `PUBLIC`/`anon`/`authenticated`, interne Authorization-Guards, Workspace-/Membership-Scoping, Entity-Scoping, AAL2 nach `DEC-016` und moegliche Privilege-Escalation-Pfade. Noch nicht ausgefuehrt; braucht eine eigene Owner-Freigabe.
