...
- `STAGING-PROVISION-01` Phase 1 am 2026-08-09 ausgefuehrt: das Staging-Projekt `NexImmo Staging` (`vhxdgchhgyzbjnogjicb`) existiert in `eu-central-1`, Organisation `deckt.` (Free), Status `ACTIVE_HEALTHY`, Zusatzkosten 0,00 €. Das Projekt ist **leer**: Migrationen `not started`, Auth `not configured`, SMTP `not configured`, GitHub-Environment `staging` `not created`, Vercel-Staging-Deploy `not started`, Golden Paths `not run`. Befund: remote laeuft PostgreSQL 17.6.1.155, lokal pinnt `supabase/config.toml` `major_version = 15` — vor dem ersten `db push` zu entscheiden. Details: `cloud/05_phase_a_log.md`.
