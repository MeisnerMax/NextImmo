...
- Realtime auf Staging: das NexImmo-Schema ist korrekt migriert und die 11 Publication-Mitgliedschaften sind vorhanden. `realtime.messages` ist eine Plattformstruktur; auf dem frischen Projekt fehlte waehrend des Testlaufs die Tagespartition, weshalb DB-seitige Broadcast-Versuche warnten. Es war dabei **kein echter WebSocket-Client verbunden** — die tatsaechliche Zustellung bleibt **UNPROVEN** und ist vor `GP-STAGING` durch einen echten Client-Connect zu beweisen. Keine Partition von Hand erzeugt, kein Eingriff in das Plattformschema. **(Geschlossen — siehe Realtime-Eintrag vom 2026-08-23/24 unten.)**
- **Realtime auf Staging am 2026-08-23/24: PASS.** Live-Zustellung mit einem echten authentifizierten `aal2`-Client bewiesen (Property-Update ohne Reload empfangen); Auth-/RLS-Negativgrenze anonym dicht (privater Entitlement-Kanal „Unauthorized", `postgres_changes` ohne Grant abgewiesen). Dabei ein echter Client-Defekt gefunden und behoben: die Reconciliation hing am ersten `postgres_changes`-Ok, ein Rejoin nach Socket-Abriss lud verpasste Aenderungen nie nach, und der Controller verschluckte Subscription-Fehler. Fix + Reconnect-Recovery remote bewiesen (`REALTIME-STAGING-FIX-01`, PR #35 → `620467e`, Remote-Closeout PASS: echte Netz-Luecke, Mutation in der Luecke, genau ein Catch-up-Readback nach Rejoin ohne Interaktion); derselbe Latch konsistent in den fuenf Schwester-Adaptern entfernt (`REALTIME-RECONNECT-CONSISTENCY-01`, PR #36 → `2473c9e`; Remote-E2E dieser fuenf Domaenen bleibt **blockiert ausschliesslich durch fehlende Staging-Daten/Permissions**, kein bekannter Defekt); der Degraded-Zustand ist als passiver UI-Hinweis sichtbar (`REALTIME-DEGRADED-UI-01`, PR #37 → `b14c879`). Golden Staging Baseline nach allen Tests exakt unveraendert (Property v11, Audit 10, Storage 0). Details: `cloud/05_phase_a_log.md`.
