...
| DEC-014, DEC-017 | external domain validation, paid provisioning | this register | relevant Phase 1 provisioning or later feature |

`DEC-015` (region) and `DEC-016` (privileged AAL2) left this index on 2026-08-08; both are
`accepted` above. `DEC-017` alone still gates every non-local environment — deciding *where*
production runs and *how* privileged access is secured does not authorize creating anything
remote. Where older documents name `DEC-015`..`DEC-017` together as a gate, only `DEC-017`
is still open; those are point-in-time records and are deliberately not rewritten.
