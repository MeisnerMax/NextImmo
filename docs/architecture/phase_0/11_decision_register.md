...
| DEC-015 | Production region is Frankfurt/EU. Paid provisioning remains a separate authorized action. | accepted | Owner decision 2026-08-02; EU data residency target | revisit only after legal/operational requirement |
| DEC-016 | Initial authentication uses passwordless email login and Supabase AAL. AAL2 is required server-side for privileged capabilities and every accepted AI proposal; `ai.use` alone does not require AAL2 and never grants domain mutation rights. | accepted | Owner decision 2026-08-02; passwordless/TOTP/logout, fail-closed client state and Property-AAL2 are locally tested | revisit only with explicit security review |
