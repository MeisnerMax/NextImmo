...
| DUP-010 | `contacts`, `tenants`, `contractors` und freie Vendor-/Counterparty-Felder bilden Parteien mehrfach und ohne gemeinsame Identitaet ab. | resolved (P2-D02, 2026-07-22) | `merge`: kanonisches Party-Modell umgesetzt — Supabase `parties`/`party_roles`/`party_contractor_details`/`party_aliases` (Migration `20260722220000_p2_d02_contacts_parties`), Contract+Adapter unter `lib/features/contacts_parties/`, deterministischer Dry-Run-Mapper `sqlite_to_postgres_contacts_parties_dry_run_mapper` (tenants/contractors/contacts → parties/party_roles/Satelliten, read-only). **Legacy-Tabellen bleiben bestehen bis Reconciliation passt** (MIG-BND-001); fachliche Rollen erhalten. | `lib/data/sqlite/migrations.dart:960`, `lib/data/sqlite/migrations.dart:3663`, `lib/data/sqlite/migrations.dart:4193`, `lib/data/sqlite/migrations.dart:798`; `supabase/migrations/20260722220000_p2_d02_contacts_parties.sql`; `lib/features/contacts_parties/` |
...
|---|---|---|---|---|---|
| OPEN-001 | Kanonisches Party-Modell fuer User, Kontakt, Mieter, Dienstleister und Kaeufer | PII, Rollen, Dubletten, Migration | getrennte fachliche Rollen, gemeinsame Party-ID | vor Phase 2 Schema | decided (2026-07-22), **implemented (P2-D02, 2026-07-22)**: Default bestaetigt (gemeinsame `parties`-ID, fachliche Rollen als `party_roles`-Zeilen, zeitlich begrenzbar) **plus** rollenspezifische Attribute in per-Rolle Satelliten-Tabellen (join auf `parties`), nicht in generischen `party_roles`-Feldern. Umgesetzt in `supabase/migrations/20260722220000_p2_d02_contacts_parties.sql` (`parties`/`party_roles`/`party_contractor_details`/`party_aliases`) + `lib/features/contacts_parties/`. |
