...
- explizite indexed rent schedule/renewal option/notice obligation, soweit nicht im aktuellen `LeaseDto`; separater Leasing-Contract, nicht Legacy-Felder imitieren. **Der Mietplan ist GELIEFERT** durch `LEASING-COMPONENTS-01` (2026-09-06/07, Migrationen 54-56): `lease_components` traegt Grundmiete, Betriebskosten- und Heizkostenvorauszahlung, Stellplatz und Sonstiges je Gueltigkeitszeitraum, mit MwSt-Modus und Satz **je Periode**, und `lease_components_as_of` loest sie auf einen Stichtag auf. Genau wie hier gefordert **kein Nachbau der Legacy-Felder**: die drei flachen Spalten auf `leases` bleiben als Vertragsbeginn-Zahlen unangetastet (DEC-029), und wo keine Komponente ein Datum abdeckt, liefert der Read *keine Zeile* - nicht null. Die Vertragsansicht zeigt den Abschnitt „Mietbestandteile“ direkt unter „Miete und Nebenkosten“, damit beide zusammen gelesen werden. **Noch offen:** Anlegen/Aendern/Beenden aus der Oberflaeche (heute nur serverseitig ueber die auditierten Commands), sowie Renewal Option und Notice Obligation, die weiterhin eigene Contracts brauchen und von diesem Paket nicht beruehrt werden.
