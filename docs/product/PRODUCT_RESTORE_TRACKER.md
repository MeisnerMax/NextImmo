...
| PROPERTY-MEDIA-DATA-01 (Prerequisite) | privates Property-Media-/Titelbild-Contract (Storage-Bucket, Version, Lifecycle, signierte Auslieferung); Documents nicht zweckentfremden | — | committed (Scope beschlossen FULL-V2-SCOPE-01) | implemented (2026-09-06, `feature/property-media-data-01`): `property_media` + privater Bucket mit entity-gescopten Storage-Policies (strenger als der Dokumenten-Bucket), zwei auditierte idempotente RPCs, keine UPDATE-/DELETE-Policy auf den Objekten, Registrierung nur gegen real hochgeladene Bytes, ein Titelbild per Unique-Index; Galerie im Objekt-Bereich. pgTAP 035 (54) und Rollback 041 (11). Offen: Titelbild in Listenzeilen (Batch-Signed-URLs), Bildreihenfolge per Drag | todo |
...
| `PROPERTY_ASSET_V2.md` | Objekt | COMMITTED | READY für Stammdaten, Lifecycle und Medien (`PROPERTY-MEDIA-DATA-01` gelandet); Titelbild in Listenzeilen: PREREQUISITE REQUIRED — Batch-Signed-URL-Read | APPROVED |
...
| `PROPERTY_ACTIVITY_REPORTS_V2.md` | Aktivität Host | COMMITTED | READY als Host mit `Protokoll` (`AUDIT-01` umgesetzt); Timeline/Berichte: PREREQUISITE REQUIRED — `PROPERTY-ACTIVITY-01`, `P2-D09` | APPROVED |
