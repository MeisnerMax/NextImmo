...
| ├ TASK-CENTER-01 | Eine Task-UI (Workspace + Objektkontext), Liste, Board (4 status-gebundene Keysets), CRUD/Status/„mir zuweisen", Bulk, Vorlagen-Tab als Leerfläche → Spec `screens/task_center.md`. **Ohne** Systemsichten/Termine/Suche/Zähler (blocked) | REBUILD/MERGE | spec_approved (2026-08-28, inkrementweise) | blocked(TASKS-NOTIFICATIONS-CORE-01) | blocked(PERMISSION-CATALOG-02) |
...
| **TASK-SCHEDULER-01 (DEBT-009)** | Vorlagen-Aggregat + serverseitiger Scheduler: recurring tasks, deadline events, scheduled notifications. Heute existiert weder `supabase/functions` noch `pg_cron`/`pg_net`. Vorher: Harvest der zehn Standardvorlagen | wiederkehrende Aufgaben, „Jetzt erzeugen", Frist-/Sammelereignisse | todo | todo |
