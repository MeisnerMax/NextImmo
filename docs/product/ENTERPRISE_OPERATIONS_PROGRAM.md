...
| **F-1** | **Kostenarten anlegen und pflegen** — *implementiert 2026-09-07, `FINANCE-COST-TYPES-01`, Migration 69, inkl. Oberfläche* | Nicht im Auftrag benannt, sondern beim Betrieb aufgefallen: `FINANCE-01a` lieferte `create_finance_account` und `update_finance_account` auditiert und freigegeben, aber die einzige Lesung, die Konten auflistet, gab die Version nicht zurück — die das Update zwingend verlangt. Kein Client konnte den Befehl also aufrufen, und die Fläche „Umlagefähigkeit" öffnete auf einer leeren Liste ohne Ausweg. Ein Feld in der Lesung, der Rest Client. § 2 BetrKV liegt als benannter Vorschlag bei, nicht als Prüfung (DEC-014) | P-5 |
...
| **P-2c** | **Basiswerte je Einheit** (fester Anteil, Personen, Miteigentumsanteil) — *implementiert 2026-09-07, `UNIT-BASIS-VALUES-01`, Migration 68, inkl. Oberfläche* | Der Wert hängt an der **Einheit**, nicht am Vertrag (OPN-DOM-001: mehrere gleichzeitige Verträge je Einheit sind erlaubt). Jeder Wert nennt seine Konvention als Pflichtangabe, weil DEC-014 für keinen der drei Maßstäbe eine Zählregel entscheidet; zwei verschiedene Konventionen ergeben keinen Nenner und werden als `mixed_conventions` verweigert. Offen und bewusst nicht hier entschieden: wie P-5 einen unterjährig geänderten Wert über die Periode aggregiert | P-5 |
