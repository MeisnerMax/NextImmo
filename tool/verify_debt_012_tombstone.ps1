...
$fixture = Join-Path $PSScriptRoot '..\supabase\tests_integration\debt_012_tombstone_setup.sql'
$target = '/tmp/neximmo-debt-012-setup.sql'
docker cp $fixture "${container}:$target" | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'DEBT-012 fixture copy failed.'
}
docker exec -i $container psql -U postgres -d postgres -v ON_ERROR_STOP=1 `
  -f $target | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'DEBT-012 fixture setup failed.'
}
...
flutter test --no-pub `
  test/integration/supabase_property_tombstone_integration_test.dart `
  "--dart-define=SUPABASE_URL=$apiUrl" `
  "--dart-define=SUPABASE_PUBLISHABLE_KEY=$publishableKey"
if ($LASTEXITCODE -ne 0) {
  throw 'DEBT-012 Supabase tombstone integration test failed.'
}
