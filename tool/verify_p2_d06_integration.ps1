...
$fixture = Join-Path $PSScriptRoot '..\supabase\tests_integration\p2_d06_setup.sql'
$target = '/tmp/neximmo-p2-d06-setup.sql'
docker cp $fixture "${container}:$target" | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D06 fixture copy failed.'
}
docker exec -i $container psql -U postgres -d postgres -v ON_ERROR_STOP=1 `
  -f $target | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D06 fixture setup failed.'
}
...
flutter test --no-pub `
  test/integration/supabase_maintenance_capex_repository_integration_test.dart `
  "--dart-define=SUPABASE_URL=$apiUrl" `
  "--dart-define=SUPABASE_PUBLISHABLE_KEY=$publishableKey"
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D06 Supabase maintenance_capex integration test failed.'
}
