...
$fixture = Join-Path $PSScriptRoot '..\supabase\tests_integration\p2_d02_setup.sql'
$target = '/tmp/neximmo-p2-d02-setup.sql'
docker cp $fixture "${container}:$target" | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D02 fixture copy failed.'
}
docker exec -i $container psql -U postgres -d postgres -v ON_ERROR_STOP=1 `
  -f $target | Out-Null
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D02 fixture setup failed.'
}
...
flutter test --no-pub `
  test/integration/supabase_party_repository_integration_test.dart `
  "--dart-define=SUPABASE_URL=$apiUrl" `
  "--dart-define=SUPABASE_PUBLISHABLE_KEY=$publishableKey"
if ($LASTEXITCODE -ne 0) {
  throw 'P2-D02 Supabase party integration test failed.'
}
