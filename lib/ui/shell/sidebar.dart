...
class Sidebar extends ConsumerStatefulWidget {
  ...
  final Set<String>? cloudPermissions;
...
class _SidebarState extends ConsumerState<Sidebar> {
  ...
  Widget build(BuildContext context) {
          ...
          (group) => _SidebarGroup(
            ...
            items: group.items
                .where(
                  (item) =>
                      widget.cloudPermissions == null
                          ? isPageAllowedForRole(item.page, role)
                          : isPageAllowedForPermissions(
                            item.page,
                            widget.cloudPermissions!,
                          ),
                )
                .map(
                  (item) => _SidebarItem(
    ...
    return AnimatedContainer(
      ...
      child: NxGlassPanel(
        ...
        fillColor: _menuBlue.withValues(alpha: 0.94),
              ...
              child:
                  collapsed
                      ? IconButton.filled(
                        tooltip:
                            widget.cloudPermissions == null
                                ? context.strings.text('New Acquisition')
                                : 'Objekte öffnen',
  ...
  Widget _buildItemTile(
    ...
    final tile = ListTile(
      ...
      leading: Icon(item.icon, color: isSelected ? primary : _menuMuted),
      title:
          collapsed
              ? null
              : Text(
    ...
    final wrapped = DecoratedBox(
      decoration: BoxDecoration(
        border:
            isSelected
                ? const Border(right: BorderSide(color: primary, width: 2))
                : null,
      ),
      child: tile,
    );
