...
class ReferenceMembersScreen extends ConsumerWidget {
  const ReferenceMembersScreen({super.key, this.embeddedInShell = false});
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    if (reference.authPhase != ReferenceAuthPhase.authenticated ||
      ...
      return embeddedInShell ? body : Scaffold(body: body);
    }
    ...
    final body = MembersAdminView(
      state: state,
      referencePresentation: !embeddedInShell,
      ...
      onInvite:
          (email, roleId) => controller.invite(email: email, roleId: roleId),
      onChangeRole:
          (membershipId, newRoleId, expectedVersion) => controller.changeRole(
            membershipId: membershipId,
            newRoleId: newRoleId,
            expectedVersion: expectedVersion,
          ),
      onUpdateStatus:
          (membershipId, newStatus, expectedVersion) => controller.updateStatus(
            membershipId: membershipId,
            newStatus: newStatus,
            expectedVersion: expectedVersion,
          ),
      onRevokeInvitation:
          (invitationId, expectedVersion) => controller.revokeInvitation(
  ...
  Widget build(BuildContext context) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          NxPageHeader(
            title: widget.referencePresentation ? 'Members' : 'Benutzer',
            breadcrumbs:
                widget.referencePresentation
                    ? const ['Reference slice', 'Members']
                    : const ['Setup & Verwaltung', 'Benutzer'],
            subtitle:
    ...
    switch (state.directoryPhase) {
      ...
      case MembersDirectoryPhase.error:
        return NxEmptyState(
          ...
          description:
              state.message ?? 'The member directory could not be loaded.',
          ...
          LayoutBuilder(
            builder: (context, constraints) {
              ...
              final role = DropdownButtonFormField<String>(
                ...
                onChanged: (value) => setState(() => _inviteRoleId = value),
              );
          ...
          builder: (context, setDialogState) {
            return AlertDialog(
              ...
              content: DropdownButtonFormField<String>(
                ...
                value:
                    roles.any((role) => role.id == selected)
                        ? selected
                        : roles.first.id,
                isExpanded: true,
                items: [
