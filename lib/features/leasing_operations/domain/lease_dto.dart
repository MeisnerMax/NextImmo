...
enum LeaseStatus {
  ...
  /// The single lawful forward step, or null once the lease is terminal.
  ///
  /// [cancelled] is deliberately not anybody's "next" step: aborting is a
  /// separate, reason-carrying decision, not the continuation of the chain.
  LeaseStatus? get nextStatus => switch (this) {
    LeaseStatus.draft => LeaseStatus.reviewed,
    LeaseStatus.reviewed => LeaseStatus.sent,
    LeaseStatus.sent => LeaseStatus.tenantSigned,
    LeaseStatus.tenantSigned => LeaseStatus.landlordSigned,
    LeaseStatus.landlordSigned => LeaseStatus.active,
    LeaseStatus.active => LeaseStatus.ended,
    LeaseStatus.ended || LeaseStatus.cancelled => null,
  };

  /// Local mirror of `private.lease_status_transition_allowed`, for offering
  /// only the move the server will accept. The server remains the authority —
  /// this never decides, it only avoids proposing a refused step. There are no
  /// backward edges in STM-005, and none are synthesised here.
  bool canTransitionTo(LeaseStatus target) {
    if (isTerminal) {
      return false;
    }
    if (target == LeaseStatus.cancelled) {
      return true;
    }
    return target == nextStatus;
  }

  /// Whether the commercial terms may still be changed in place. Mirrors
  /// `update_lease`'s editable states: from the first signature on, a lease is
  /// binding and a change of terms is a new lease, not an edit of this one.
  bool get isEditable =>
      this == LeaseStatus.draft ||
      this == LeaseStatus.reviewed ||
      this == LeaseStatus.sent;
}
