...
abstract interface class RentRollPort {
  ...
  ///
  /// Deliberately a port method rather than an arithmetic helper on the screen:
  /// the rule that decides which lease counts belongs next to the one that
  /// builds a snapshot, so the two cannot drift. In the cloud adapter this is
  /// one server-side read; in the local adapter it is the legacy mirror of the
  /// same rule, and it dies with that backend.
