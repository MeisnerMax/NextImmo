...
select is(
  (select count(*)::integer
   ...
   where namespace.nspname = 'public'
     ...
     and function.prosecdef),
  88,
  'SR-20: the public SECURITY DEFINER inventory is still 88 -- update this expectation deliberately when it changes'
);
...
select is(
  (select count(*)::integer
   ...
   where namespace.nspname in ('public', 'storage', 'realtime')),
  50,
  'SR-22: the client-reachable policy inventory is still 50 -- update this expectation deliberately'
);
